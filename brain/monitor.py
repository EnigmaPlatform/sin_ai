    ...
    def _plot_progress(self) -> None:
        ...
        try:
            ...
            plt.plot(self.current_log["epochs"], self.current_log["train_loss"], 
                    label=f"Train (best: {self.best_metrics['train_loss']:.4f})")
            
            if self.current_log["val_loss"]:
                plt.plot(self.current_log["epochs"], self.current_log["val_loss"], 
                        label=f"Val (best: {self.best_metrics['val_loss']:.4f})")
            ...
            plt.tight_layout()
            plt.savefig(self.log_dir / "training_progress.png", dpi=300)
            plt.close()
