...
class VocabularyLearningSystem:
    ...
    def _parse_definition(self, html_content, word):
        ...
        if not BEAUTIFUL_SOUP_AVAILABLE:
            logger.warning("BeautifulSoup не доступен для парсинга определений")
            return None
        ...
        try:
            ...
            for header in headers:
                ...
                if 'русский' in header_text.lower():
            ...
            definition_keywords = ['значение', 'определение', 'это']
            
            while current_element and current_element.name not in ['h2', 'h3']:
                if current_element.name == 'ol':  # Список определений
                    list_items = current_element.find_all('li')
                    for item in list_items:
                        text = item.get_text().strip()
                        if text and len(text) > 10:
                            # Очищаем от лишних символов
                            clean_text = re.sub(r'\[\d+\]', '', text)  # Убираем сноски
            ...
            if not definitions:
                # Попробуем найти определения другим способом
                paragraphs = soup.find_all('p')
                for p in paragraphs:
                    text = p.get_text().strip()
                    if any(keyword in text.lower() for keyword in definition_keywords):
                        # Извлекаем текст после ключевых слов
                            ...
                            if keyword in text.lower():
                                ...
                                if len(parts) > 1:
                                    ...
                                    if definition_text and len(definition_text) > 10:
                                        clean_text = re.sub(r'\s+', ' ', definition_text)
                                        definitions.append(clean_text)
                                break
    ...
    def _extract_examples(self, soup, start_section):
        ...
        try:
            ...
            example_sections = soup.find_all(['dd', 'blockquote', 'div'])
            for section in example_sections:
                # Проверяем, содержит ли секция примеры
                text = section.get_text().strip()
                if text and len(text) > 15 and ('пример' in text.lower() or '«' in text):
                    # Очищаем от лишних символов
                    clean_text = re.sub(r'\s+', ' ', text)
                    # Убираем лишние теги и сноски
    ...
    def auto_learn_vocabulary(self, limit=50, delay=2.0):
        ...
        try:
            while learned_count < limit and self.learning_queue:
                ...
                # Показываем прогресс
                if learned_count % 10 == 0 and learned_count > 0:
        ...
        if os.path.exists(self.model_file):
            try:
                ...
                for neuron_id, neuron_data in model_data['neurons'].items():
                    ...
                    if 'activation_history' in neuron_data:
            ...
            logger.warning(f"⚠️ Модель не найдена в {self.model_file}")
            return False
...
/chat <сообщение>
  Описание: Обычный чат с нейросетью Sin
  Пример: /chat Привет, как дела?

/learn <текст>
  Описание: Обучение нейросети на указанном тексте
  Пример: /learn Искусственный интеллект - это область компьютерных наук...

/file <путь>
  Описание: Обучение на содержимом файла (txt, docx, pdf, json)
  Пример: /file /home/user/document.txt

/url <ссылка>
  Описание: Обучение на содержимом веб-страницы
  Пример: /url https://ru.wikipedia.org/wiki/Искусственный_интеллект

/auto <минуты>
  Описание: Автоматическое обучение в течение указанного времени
  Пример: /auto 30 (автообучение 30 минут)

📚 РАБОТА С ЛЕКСИКОЙ:

/define <слово>
  Описание: Поиск значения слова в Wiktionary
  Пример: /define философия

/add_words <слова>
  Описание: Добавление слов в очередь для изучения (через запятую)
  Пример: /add_words любовь, ненависть, счастье, грусть

/learn_words <лимит>
  Описание: Автоматическое изучение указанного количества слов
  Пример: /learn_words 50 (изучить 50 слов)

/vocab_stats
  Описание: Показ статистики изучения лексики

📊 СИСТЕМНЫЕ КОМАНДЫ:

/stats
  Описание: Показ полной статистики нейросети

/report
  Описание: Комплексный отчет о состоянии всех систем

/save
  Описание: Сохранение текущего состояния модели

/load
  Описание: Загрузка сохраненного состояния модели

/user <id>
  Описание: Смена пользователя (для персонализации)
  Пример: /user user123

/help
  Описание: Показ этого справочного сообщения

/FAQ
  Описание: Показ часто задаваемых вопросов

/exit
  Описание: Завершение работы с нейросетью

💡 СОВЕТЫ ПО ИСПОЛЬЗОВАНИЮ:

1. Начните с простого общения: /chat Привет!
2. Для обучения используйте качественные тексты
3. При изучении слов указывайте реальный лимит
4. Регулярно сохраняйте модель: /save
5. Проверяйте статистику: /stats

⚠️ ВАЖНО:

- Изучение слов требует подключения к интернету
- Автоматическое обучение может занять время
- Некоторые функции требуют установленных дополнительных библиотек
"""
        print(faq_text)
